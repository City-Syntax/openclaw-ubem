...
## Known IDF baseline issues (Climate Studio 2.0 defaults)
- **Infiltration ACH**: 6.0 in all IDFs. Unrealistically high for sealed NUS buildings. Target: 0.5 ACH via calibration.
- **Cooling setpoint**: IDFs may show 25°C (Climate Studio default) but setpoints reflect each building's actual operational settings. **Never change Cooling_Setpoint_C** — not in pre-flight, not in calibration, not ever. Exception: Compass may recommend a +1 °C increase (capped) to the facilities team as an intervention — this is a human-actioned recommendation only, never an agent IDF write.
...
- **No iteration cap** — keep iterating (with human approval each time) until CVRMSE ≤ 15% AND NMBE ≤ ±5%
- Stop only when all calibration parameters hit their bounds without convergence → flag for human review
...
1. Infiltration_ACH: reduce from Climate Studio default (typically 6.0 → 0.5)
2. Equipment_W_per_m2: adjust based on building type and NMBE direction
3. Lighting_W_per_m2: adjust if still over/under-predicting after equipment fix
